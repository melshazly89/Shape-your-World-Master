...
unsigned long Convert(unsigned long sample){
  return (sample*2000/4096)+1;  // replace this line with real code
}
...
void SysTick_Handler(void){ 
  ADCdata=ADC0_In();                   //Reading 12-bit binary ADC sample
	Distance=Convert(ADCdata);           //Convert a 12-bit binary ADC sample into Distance
}
...
int main(void){ 
  TExaS_Init(ADC0_AIN1_PIN_PE2, UART0_Emulate_Nokia5110_NoScope);  //initialize Grader
  ADC0_Init();                                                     // initialize ADC0, channel 1, sequencer 3
  Nokia5110_Init(); 	                                             // initialize Nokia5110 LCD (optional)
	SysTick_Init();                                                  // initialize SysTick for 40 Hz interrupts
...
// print a welcome message  (optional)
  while(1){ 
		// read mailbox
		UART_ConvertDistance(Distance);    //Converts a 32-bit distance into an ASCII string
    Nokia5110_SetCursor(0, 3);         //Set The Cursor
